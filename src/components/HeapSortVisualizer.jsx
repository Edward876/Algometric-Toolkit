...
// Styled components - reusing styles with a unique color theme for heap sort
...
const Title = styled.h1`
  ...
  &::before {
    ...
    right: 0;
    bottom: -10px;
    background: radial-gradient(ellipse at center, rgba(247, 151, 30, 0.15) 0%, rgba(0, 0, 0, 0) 70%);
    z-index: -1;
    border-radius: 50%;
  }
`;
...
const Button = styled(motion.button)`
  background: ${props => props.primary ? 'linear-gradient(90deg, #F7971E 0%, #FFD200 100%)' : '#334155'};
  border: none;
  padding: 0.75rem 1.5rem;
  border-radius: 8px;
  color: ${props => props.primary ? '#1e293b' : 'white'};
  font-weight: bold;
  cursor: pointer;
...
const VisualizationContainer = styled.div`
  ...
  &::after {
    ...
    right: 0;
    bottom: 0;
    background: linear-gradient(135deg, rgba(247, 151, 30, 0.05) 0%, rgba(255, 210, 0, 0.05) 100%);
    pointer-events: none;
  }
`;
...
const ArrayBar = styled(motion.div)`
  ...
  background: ${props => {
    if (props.heapify) return '#F59E0B';
    if (props.comparing) return '#FBBF24';
    if (props.swapping) return '#F97316';
    if (props.sorted) return '#34D399';
    if (props.root) return '#FCD34D';
    return '#60A5FA';
  }};
  ...
  font-size: 0.8rem;
  position: relative;
  box-shadow: ${props => (props.comparing || props.swapping || props.heapify || props.root) ? '0 0 15px rgba(245, 158, 11, 0.5)' : 'none'};
...
const StepCard = styled(motion.div)`
  background: ${props => props.active ? 'linear-gradient(90deg, #F7971E 0%, #FFD200 100%)' : 'rgba(51, 65, 85, 0.8)'};
  ...
  cursor: pointer;
  box-shadow: ${props => props.active ? '0 10px 20px rgba(247, 151, 30, 0.2)' : 'none'};
  border: 1px solid ${props => props.active ? 'rgba(247, 151, 30, 0.5)' : 'rgba(255, 255, 255, 0.05)'};
`;
...
const Tab = styled.button`
  background: ${props => props.active ? 'linear-gradient(90deg, #F7971E 0%, #FFD200 100%)' : 'transparent'};
  color: ${props => props.active ? '#1e293b' : '#94a3b8'};
  border: none;
  padding: 1rem 1.5rem;
  cursor: pointer;
  ...
  &:hover {
    color: ${props => props.active ? '#1e293b' : '#e2e8f0'};
    background: ${props => props.active ? 'linear-gradient(90deg, #F7971E 0%, #FFD200 100%)' : 'rgba(51, 65, 85, 0.3)'};
  }
`;
          ...
          <AnimatePresence>
            {steps.length > 0 && currentStep < steps.length && steps[currentStep].array.map((value, index) => (
              <ArrayBar
                ...
                heapify={steps[currentStep].heapify?.includes(index)}
