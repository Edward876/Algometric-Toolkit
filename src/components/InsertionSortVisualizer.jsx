...
const ArrayBar = styled(motion.div)`
  ...
  background: ${props => {
    if (props.key_element) return '#EC4899'; // pink for key element
    if (props.comparing) return '#FBBF24'; // yellow for comparing
    if (props.current) return '#A78BFA'; // purple for current
  ...
  position: relative;
  box-shadow: ${props => {
    if (props.key_element) return '0 0 15px rgba(236, 72, 153, 0.5)';
    if (props.comparing) return '0 0 15px rgba(251, 191, 36, 0.5)';
    if (props.current) return '0 0 15px rgba(167, 139, 250, 0.5)';
