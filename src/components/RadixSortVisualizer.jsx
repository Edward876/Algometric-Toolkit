...
const ArrayBar = styled(motion.div)`
  ...
  background: ${props => {
    ...
    if (props.processing) return '#EC4899';
    if (props.bucket) return '#8B5CF6';
  ...
  font-size: 0.8rem;
  position: relative;
  box-shadow: ${props => props.current || props.processing || props.bucket ? '0 0 15px rgba(251, 191, 36, 0.5)' : 'none'};
`;
        ...
        {steps.length > 0 && currentStep < steps.length && steps[currentStep].buckets && (
          <BucketsContainer>
            {Array.from({ length: 10 }).map((_, i) => (
              <Bucket key={i}>
                <BucketTitle>Bucket {i}</BucketTitle>
                <BucketContent>
